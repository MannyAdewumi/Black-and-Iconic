...
const Faq = () => {
  ...
  return (
    <section className="bg-black text-white px-6 md:px-12 py-20 font-poppins">
      <div className="max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-semibold mb-10">
          Frequently asked questions
        </h2>
        <div className="space-y-4">
          {frequents.map((freq, index) => (
            <div
              key={index}
            ...
            >
              <button
                ...
                className="w-full flex justify-between items-center text-left text-base md:text-[16px] font-medium hover:text-gray-400 transition-colors"
              >
                <span className='font-medium'>{freq.question}</span>
                <span className="ml-4">
                  {activeIndex === index ? (
              ...
              {activeIndex === index && (
                <p className="mt-4 text-sm text-gray-400 transition-all duration-300">
                  {freq.answer}
                </p>
              )}
            </div>
